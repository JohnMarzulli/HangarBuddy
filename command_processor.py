...
class CommandProcessor(object):
    ...
    def run_hangar_buddy(
        ...
        while True:
            ...
            self.__run_servicer__(
                self.__process_pending_text_messages__,
    ...
    def __clear_existing_messages__(
        self
    ):
        """
        ...
        if num_deleted > 0:
            ...
            self.__logger__.log_info_message(
                "{} old message cleared from SIM Card".format(num_deleted))

    ##############################
    # --- Status builders
    ##############################

    def __get_heater_status__(
        self
    ):
        """
    ...
    def __get_temp_probe_status__(
        ...
        if self.__sensors__.current_temperature_sensor_reading is not None:
            return "TEMP: {}F".format(self.__sensors__.current_temperature_sensor_reading)

        return "Temp probe not enabled."

    def __get_uptime_status__(
        self
    ):
        """
    ...
    def __get_full_status__(
        ...
        try:
            status += '{}\n'.format(self.__get_heater_status__())
            status += '{}\n'.format(self.__get_gas_sensor_status__())
            status += '{}\n'.format(self.__get_light_status__())
            status += '{}\n'.format(self.__get_temp_probe_status__())
            status += '{}\n'.format(self.__get_fona_status__())
    ...
    def __heater_turned_on_callback__(
        self
    ):
        """
    ...
    def __heater_turned_off_callback__(
        self
    ):
        """
    ...
    def __heater_max_time_off_callback__(
        self
    ):
        """
    ...
    def __queue_message_to_all_numbers__(
        self,
        message
    ):
        """
    ...
    def __handle_on_request__(self, phone_number):
        ...
        self.__logger__.log_info_message(
            "Received ON request from {}".format(phone_number))

        if self.__relay_controller__.is_relay_on():
            return CommandResponse(
                text.NOOP,
                "Heater is already ON, {}".format(
        ...
        return CommandResponse(
            text.HEATER_ON_COMMAND,
            "Heater turning on for {} minutes".format(
                self.__configuration__.max_minutes_to_run))

    def __handle_off_request__(
        self,
        phone_number
    ):
        """
        ...
        if self.__relay_controller__.is_relay_on():
            try:
                return CommandResponse(
                    text.HEATER_OFF_COMMAND,
                    "Turning heater off with {}".format(
                        self.__relay_controller__.get_heater_time_remaining()))
            except:
                return CommandResponse(
                    text.ERROR,
    ...
    def __handle_status_request__(
        ...
        """
        self.__logger__.log_info_message(
            "Received STATUS request from {}".format(phone_number))

        return CommandResponse(
            text.FULL_STATUS_COMMAND,
            self.__get_full_status__())

    def __handle_help_request__(
        self,
        phone_number
    ):
        """
        ...
        """
        self.__logger__.log_info_message(
            "Received HELP request from {}".format(phone_number))

        return CommandResponse(
            text.HELP_COMMAND,
            self.__get_help_status__())

    def __handle_uptime_request__(
        self,
        phone_number
    ):
        """
    ...
    def __handle_temperature_request__(
        self,
        phone_number
    ):
        """
        ...
        return CommandResponse(
            text.TEMPERATURE_COMMAND,
            self.__get_temp_probe_status__())

    def __handle_cell_status_request__(
        self,
        phone_number
    ):
        """
    ...
    def __handle_lights_request__(
        ...
        return CommandResponse(
            text.LIGHTS_COMMAND,
            self.__get_light_status__())

    def __handle_shutdown_request__(
        self,
        phone_number
    ):
        """
        ...
        return CommandResponse(
            text.SHUTDOWN_COMMAND,
            "Received SHUTDOWN request from {}".format(
                phone_number))

    def __handle_restart_request__(
        self,
        phone_number
    ):
        """
    ...
    def __get_command_response__(
        ...
        return CommandResponse(
            text.HELP_COMMAND,
            "INVALID COMMAND\n{}".format(self.__get_help_status__()))

    def __handle_gas_ok__(
        self,
        gas_sensor_status
    ):
        """
    ...
    def __monitor_gas_sensor__(
        ...
        self.__logger__.log_info_message(
            "Detected: {0}, Level={1}".format(detected, current_level))
    ...
    def __monitor_fona_health__(
        ...
        self.__logger__.log_info_message(
            "GSM Battery={0}% Volts={1}".format(
                cbc.get_percent_battery(),
                cbc.get_voltage()))
    ...
    def __service_gas_sensor_queue__(
        self
    ):
        """
    ...
    def __process_pending_text_messages__(
        self
    ):
        """
        ...
        if total_message_count > 0:
            ...
            for message in sorted_messages:
                ...
                if delta_startup < 0:
                    old_message = "MSG was sent {} before startup.".format(
...
def test_valid_commands():
    ...
    for command in VALID_COMMANDS:
        ...
        print("Testing {0} {1}".format(command, message))
...
if __name__ == '__main__':
    ...
    CONTROLLER = CommandProcessor(
        CONFIG,
        HangarBuddyLogger(logging.getLogger("Controller")))
